...
function t796_init(recId) {
    ...
    if (shapeRecId) {
        ...
        var excludesBlocks = [215, 316, 390, 651, 702, 706, 708, 750, 756, 766, 825, 862, 868, 943, 945];
        var excludes = '';
...
function t796_setColor(rec, shape) {
    if (shape.getAttribute('data-fill-color'))
        return;
    var nearestBlock;
...
function t796_addDivider(rec, shape) {
    ...
    if (cover || zeroBlock) {
        if (cover) {
            ...
            if (coverFilter)
                coverFilter.insertAdjacentElement('afterend', shape)
        }
        if (zeroBlock) {
        ...
        var excludesBlocks = [125, 331, 554, 746, 754, 776, 778, 786, 858, 896, 897, 924, 915, 943, 945, 951, 1025];
...
function t786__showMore(recid) {
    ...
    if (isNaN(cardsShowLimit) || cardsShowLimit <= 0)
        return;
...
function t786_copyTypography(recid) {
    ...
    if (!rec)
        return;
    var productTitle = rec.querySelector('.t786__title');
    var productDescription = rec.querySelector('.t786__descr');
    if (productTitle)
        var titleStyle = productTitle.getAttribute('style');
    if (productDescription)
        var descriptionStyle = productDescription.getAttribute('style');
    var popupTitles = rec.querySelectorAll('.t-popup .t786__title');
    Array.prototype.forEach.call(popupTitles, function(title) {
        title.setAttribute('style', t786_removeSizeStyles(titleStyle ? titleStyle : ''))
    });
    var popupDescription = rec.querySelectorAll('.t-popup .t786__descr, .t-popup .t786__text');
    Array.prototype.forEach.call(popupDescription, function(description) {
        description.setAttribute('style', t786_removeSizeStyles(descriptionStyle ? descriptionStyle : ''))
    })
}
if (!Element.prototype.matches) {
    Element.prototype.matches = Element.prototype.matchesSelector || Element.prototype.msMatchesSelector || Element.prototype.mozMatchesSelector || Element.prototype.webkitMatchesSelector || Element.prototype.oMatchesSelector
